...
最后一行是 `--profile headless` 下的实际差别：headless 过去在 root agent 回合结束时结束运行——派发了后台 subagent 然后让出的 root 已经结束了回合，于是运行退出，子 agent 的工作丢失；而 Lead 会在 `wait_agent` 中阻塞。现在 headless 运行器接受可选的 `ctx.headlessSettlement` 服务；[`agent-team-settlement`](packages/experimental/agent-team-settlement/README.zh.md) 在 Team 任务板之上提供它，`agent-team` 可以把普通的 `subagent` 运行记录到该任务板上（`trackSubagentRuns`），运行不得不放弃的工作会被记录为 `lost` 而不是直接丢掉。两种方式都还没有类型化产物或由 harness 判定的"完成"；这些记录在 [zhengxuyu/mllm-benchs](https://github.com/zhengxuyu/mllm-benchs/blob/main/DEFECTS.md)。
