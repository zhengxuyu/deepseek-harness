...
A note is a message whose recipient is a task: `noteTask` records it on the task (`notes`), mails it to the task's current owner, and the brief of whoever claims the task later lists it. A note that names another live task's declared output path holds that task (`holds`): `complete` answers `TEAM_TASK_HELD` until the Lead clears each hold with `acknowledge`, naming the note. Notes need a live task.

The board holds one node per piece of work. `create` and an `edit` that renames refuse a subject another live (`pending`, `in_progress`, or `lost`) task carries, comparing case- and whitespace-insensitively, with `TEAM_TASK_DUPLICATE_SUBJECT` naming that task and its status: the caller depends on it, edits it, or reopens it when it is lost. A completed or deleted task frees its subject, so a second pass may reuse the name.
