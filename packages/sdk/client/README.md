...
`provider` and `model` are required, with no default: the handshake sends the pair verbatim, and which routes resolve is decided by the runtime the caller launches — so this SDK has no basis for choosing one. Both are checked at construction (an absent or blank half throws a `TypeError` naming the option) rather than surfacing later as a handshake failure against a route nobody chose.
